...
export const DashboardHeader = ({ user, activities, transactions }: Props) => {
  const { currency, locale, module_features } = useUserPreferences();
  ...
  const renderNarrative = () => {
    if (isTimelineEnabled && isFinanceEnabled) {
      return (
        <>
    ...
    return (
      <>
        "{t('dashboard.welcome_back')}. {t('dashboard.narrative_default_msg')}"
      </>
    );
  };
